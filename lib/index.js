...
import { newUserText, parseFacts, compressRules, buildInjectionBlock } from './automation.js'
...
function recentDigestSync(store, maxChars, split, mode = 'recent') {
  ...
  const sorted = [...records.values()]
    .filter((r) => r.superseded !== true) // corrected facts never re-inject
    ...
    const routes = {
      ...
      '/api/memory/settings': async (req, res) => {
        if (req.method === 'POST') {
          ...
          const patch = {}
          for (const key of ['autoSummarize', 'compressWithLLM', 'injectContext', 'requireApprovalForWrite', 'charLimit']) {
