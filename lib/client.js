...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function MemoryPanel(props) {
			...
			// ── 按逻辑分类（tag）分组展示 ──────────────────────────────────────
			// 类别 = 记录的第一个 tag（无 tag 归入「未分类」）；已知类别有中文
			// 标签，未知 tag 显示为 #tag。
			const UNTAGGED = "__untagged__";
			const CATEGORY_ORDER = ["preference", "decision", "constraint", "project", "correction", "personal"];
			const categoryOf = (r) => (Array.isArray(r.tags) && r.tags.length > 0 ? r.tags[0] : UNTAGGED);
			const categoryLabel = (cat) => {
				if (cat === UNTAGGED) return t("panel.untagged");
				const known = t(`tag.${cat}`);
				return known && !known.startsWith("tag.") ? known : `#${cat}`;
			};
			const categoryRank = (cat) => {
				if (cat === UNTAGGED) return 99;
				const i = CATEGORY_ORDER.indexOf(cat);
				return i >= 0 ? i : 50;
			};
			// Group current records by category (search/scope filtered already).
			...
			const visibleCats = catFilter === "all" ? cats : cats.filter((c) => c === catFilter);
			const sections = visibleCats.map((cat) => {
				...
				return react.createElement("div", { key: cat },
					react.createElement("div", { style: { display: "flex", alignItems: "baseline", gap: 6, margin: "8px 0 2px", borderBottom: "1px solid rgba(128,128,128,.25)" } },
						react.createElement("span", { style: { fontWeight: 600, fontSize: 12 } }, categoryLabel(cat)),
			...
			const body = showSettings
				...
				: react.createElement("div", null,
					editor,
					importRow,
					loading ? react.createElement("div", null, t("panel.loading")) : ((records && records.length > 0) ? sections : react.createElement("div", { style: { opacity: .6 } }, t("panel.empty"))),
				);
