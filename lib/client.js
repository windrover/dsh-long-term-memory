 ...
 *      priority -1, shadowing the shipped tool-details panel): list, search,
	...
	factory: (require) => {
		...
		function MemoryPanel(props) {
			...
			const doExport = react.useCallback(async () => {
				try {
					...
					const bundle = JSON.stringify({ version: 1, records: (data.results || []).map((r) => ({ scope: r.scope, content: r.content, tags: r.tags || [] })) }, null, 2);
			...
			const rows = (records || []).map((r) =>
				react.createElement("div", { key: r.id, style: { borderBottom: "1px solid rgba(128,128,128,.2)", padding: "6px 0" } },
					react.createElement("div", { style: { display: "flex", gap: 6, alignItems: "center" } },
						...
						react.createElement("button", { onClick: () => setEditing({ id: r.id, scope: r.scope, content: r.content, tags: (r.tags || []).join(", ") }) }, t("panel.edit")),
						react.createElement("button", { onClick: () => removeRecord(r.id) }, t("panel.delete")),
					),
			...
			const addRow = react.createElement("div", { style: { marginBottom: 8 } },
				react.createElement("button", { onClick: () => setEditing({ scope: "global", content: "", tags: "" }) }, t("panel.add")),
			);
			...
			const toolbar = react.createElement("div", { style: { display: "flex", gap: 6, marginBottom: 8 } },
				react.createElement("button", { onClick: doExport }, t("panel.export")),
				react.createElement("button", { onClick: () => setImportOpen(!importOpen) }, t("panel.import")),
			);

			return react.createElement("div", { style: { padding: 10, fontSize: 13, overflowY: "auto", height: "100%" } },
				react.createElement("div", { style: { fontWeight: 600, marginBottom: 8 } }, t("panel.title")),
				...
				editor,
				importRow,
				usage > 0 && react.createElement("div", { style: { fontSize: 11, opacity: .6, marginBottom: 6 } }, t("panel.usage", { used: usage, limit })),
		...
		function SettingsCard(props) {
			...
			return react.createElement("div", { style: { padding: 12 } },
				...
				react.createElement("button", { onClick: save, style: { marginTop: 10 } }, t("panel.save")),
			);
		}
		//#endregion

		//#region lib/types/client/apply.js
		const inject = ["@deepseek-ai/dsh-client-runtime", "@deepseek-ai/dsh-client-locale", "@deepseek-ai/dsh-client-ui-layout"];
		function apply(ctx) {
			...
			}, (props) => react.createElement(MemoryPanel, { t: bindT(ctx, props) })));
			...
			ctx.slots.inject("settings.plugin.item", () => ctx.slots.register({
				...
				locale: NS,
				inject: () => ({}),
			}, (props) => react.createElement(SettingsCard, { t: bindT(ctx, props) })));
		}
		function bindT(ctx, props) {
			// Prefer the registered dictionary when available; fall back to zh.
			const dict = (() => {
				try {
					const d = ctx.locale?.dictionary?.(NS);
